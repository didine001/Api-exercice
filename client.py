...
def search_artists_by_name(artist_name):
    response = requests.get(f"{FAST_API_URL}/artists/{artist_name}")  # we use the endpoint created in api.py and we use the corresponding route to get the data.
    return (response.json())  # We format on json because if we don't it returns an object.
...
def get_top_tracks():
    response = requests.get(f"{FAST_API_URL}/top_tracks/")
    return response.json()
